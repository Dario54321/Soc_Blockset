...
addpath('C:\Users\lenovo GAME\Desktop\Prove\MPC\wip_DAR');
% traces' matfiles
cd(DataDir);
...
% LongVel_Follwr
ts = getsamples(data.LongVel_Follwr,2:length(data.LongVel_Follwr.Time));
ds = ds.addElement(ts, 'LongVel_Follwr');
% PosErrFollwr1
ts = getsamples(data.PosErrFollwr1,2:length(data.PosErrFollwr1.Time));
ds = ds.addElement(ts, 'PosErrFollwr1');

% VelErrFollwr1
ts = getsamples(data.VelErrFollwr1,2:length(data.VelErrFollwr1.Time));
ds = ds.addElement(ts, 'VelErrFollwr1');

% V2V_BSM
ts = getsamples(data.V2V_BSM,2:length(data.V2V_BSM.Time));
ds = ds.addElement(ts, 'V2V_BSM');
