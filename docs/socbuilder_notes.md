...
## Aggiornamento — ripreso da zero con Register Channel, e stavolta si arriva davvero al bitstream

Il punto sopra si fermava a `IPCoreVersion` costruendo il modello sul canale **AXI4-Stream**. In una sessione successiva, ripartendo da zero (nuova cartella `prove_2/`, tre file separati `MatMul_Processor.slx`/`MatMul_FPGA.slx`/`MatMul_Top.slx`, costruiti in GUI) con il canale **Register Channel/AXI4-Lite** invece di AXI4-Stream, `socModelBuilder`/`buildModel` è arrivato fino a un **bitstream reale**. Il canale diverso ha esposto un insieme di regole in parte sovrapposto, in parte nuovo rispetto alle 4 di sopra — ognuna trovata dall'errore reale, non per tentativi:

1. **`ProjectFolder` non supporta path con spazi** — lavorare sempre in una cartella scratch senza spazi (usata `D:\SocBuilderBuild\`), non nel percorso utente reale.
2. **`FPGA or processor model not detected` (`soc:msgs:NothingToGen`)**, due cause insieme: (a) il modello Processore dev'essere incapsulato in un **Subsystem con un blocco `proctasklib/Task Manager`** al suo interno — un Model Reference nudo a livello radice, come per l'FPGA, non basta; (b) le stesse condizioni della regola 1 di sopra (`HardwareBoard=None`, `ProdHWDeviceType=ASIC/FPGA->ASIC/FPGA`, solver Fixed-step) sul modello FPGA.
3. **Non mutare a mano il JSON `AllTaskData` di un Task Manager esistente** per cambiarne il tipo di task (es. da Event-driven ad altro) — rompe una struttura "Variant Source" interna del blocco mascherato. Aggiungere invece un'istanza **fresca** da libreria: il default (`Timer-driven`, `taskEventSource=Internal`) è già adatto a Register Channel, che non ha eventi/interrupt come AXI4-Stream.
4. Il Model Reference del Processore non ha porta trigger di default — va aggiunto un blocco **Trigger** (`simulink/Ports & Subsystems/Trigger`, `TriggerType=function-call`) alla radice del modello Processore per farla comparire (si legge poi da `PortHandles.Trigger`, non `.Inport`).
5. **Sample time**: il modello FPGA con `SolverType=Fixed-step` ma `FixedStep=auto` dà `sample-time independent` — serve un valore esplicito (usato `1e-8`, coerente col target 100MHz).
6. **La regola 2 di sopra ("Only subsystems can be IP") si riapplica identica** anche nel percorso SoC Builder con Register Channel, non solo nell'IP Core Generation diretta.
7. **Register Channel accetta solo vettori piatti** (non `[3 3]`) — già noto, ma qui si è scoperto che i blocchi Reshape per convertire piatto↔matrice **non possono stare liberi né nel Top né alla radice del modello FPGA** (violano di nuovo la regola 6). Il reshape va portato **dentro** il modello FPGA, con le porte radice diventate piatte (9 elementi) e il reshape raggruppato **insieme al DUT** in un unico Subsystem.

**Risultato finale**: ELF ARM reale compilato e linkato (toolchain Linaro AArch32, 0 errori), VHDL reale generato per il DUT, IP Core Xilinx pacchettizzato con interfaccia AXI4-Lite completa, progetto Vivado block-design generato. Un ultimo blocco **non di modellazione ma di ambiente**: la build lanciava 6 job Vivado in parallelo su una macchina a 6 core, causando fallimenti di sintesi su 6 IP ausiliari (clock wizard, reset generator, crossbar, JTAG-AXI, protocol converter) per un errore Windows di spawn processo (`Impossibile eseguire il programma specificato`, nascosto sotto `input in flex scanner failed`) — non legato al nostro design (il nostro IP sintetizzava correttamente anche nel giro fallito). Risolto rilanciando i run falliti sequenzialmente (`launch_runs -jobs 1`). **Bitstream generato con successo** (`write_bitstream`, ~957 KB, 0 errori). Utilizzo del sistema completo: `1375 LUT (2.58%), 1875 Registri (1.76%), 27 DSP (12.27%)` — i 27 DSP coincidono esattamente col conteggio standalone del DUT (vedi `hdl_findings.md`, Risultato 4).

**Limiti onesti**: il bitstream usa il package `clg484` (ZedBoard, l'unica board nativamente supportata da questo meccanismo SoC Builder), non `clg400` (Pynq-Z1 reale) — non è direttamente flashabile sulla scheda vera così com'è. Nessun deployment su hardware reale è stato fatto. Le correzioni strutturali (Task Manager, Trigger, DUT wrapper, porte piatte) sono state applicate su copie di lavoro in `D:\SocBuilderBuild\matmul_build\` (cartella scratch, non versionata) — i file `.slx` in `prove_2/` in questo repository riflettono lo stato **prima** di queste correzioni (la struttura verificata funzionalmente in simulazione, Fase 1), non ancora quella completa fino al bitstream.

## Aggiornamento — progresso su `validate_refdesign()` di `HDL_Test/Prova_2` (G12b), board file PYNQ-Z1 installati

Nota di servizio per chi riprende il filone `Prova_2` (Carmine): letti per intero tutti i documenti in `HDL_Test/Prova_2/docs/`, e su richiesta esplicita dell'utente sono stati scaricati e installati i board file reali della PYNQ-Z1 da `github.com/cathalmccabe/pynq-z1_board_files` (branch `master`, `pynq-z1/1.0/`) in `D:\Xilinx\Vivado\2022.1\data\boards\board_files\pynq-z1\1.0\` (percorso di default di Vivado, nessun `board.repoPaths` necessario). **SHA-256 di `board.xml`/`part0_pins.xml`/`preset.xml` verificato identico** a `hdlplugins/board_files_manifest.m`.

Rilanciando `validate_refdesign()` (istruzioni in [`24_REFERENCE_DESIGN.md` §24.5](../HDL_Test/Prova_2/docs/24_REFERENCE_DESIGN.md)) su questa macchina (Vivado 2022.1 reale, non 2026.1):

- **Prima dei board file**: `esito='fallita'` — `ERROR: [Board 49-71] The board_part definition was not found for www.digilentinc.com:pynq-z1:part0:1.0`.
- **Dopo**: `esito='parziale'` (non ancora `completa`) — ma con **progresso reale**: il preset della board si applica correttamente, `ddrPart='MT41J256M16 RE-125'` (il DDR3 vero della Pynq-Z1, come atteso da §24.2), `fclk0='100'` MHz. Il blocco residuo, dal log completo (non solo il riassunto stampato da `validate_refdesign`, che per il ramo `parziale` mostra un testo fisso pensato per la 2026.1 di Carmine, fuorviante qui): `ERROR: [BD 5-313] Found unsupported IP 'xilinx.com:ip:axi_interconnect:1.7' in design.` Il file IP **esiste realmente su disco** in questa installazione (`D:\Xilinx\Vivado\2022.1\data\ip\xilinx\axi_interconnect_v1_7\` e anche `v2_1`) — quindi non è un file mancante. "Unsupported" (non "not found") in Vivado di solito indica licenza o catalogo IP non aggiornato per la sessione (`update_ip_catalog`), non ancora verificato a fondo. **Prossimo passo concreto per chi riprende**: controllare licenza/catalogo IP per `axi_interconnect` su questa specifica installazione 2022.1 prima di rilanciare.
