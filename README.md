...
- **NUOVO BLOCCO, distinto dal precedente, conclusione definitiva: il bitstream FPGA richiede interazione GUI reale.** Tentato `BuildType='Processor and FPGA'`/`'FPGA only'`: il software si ricompila regolarmente, ma il build fallisce sempre con `Unrecognized field name "bit_file"`. **Causa**: il subsystem `FPGA` del modello non è mai stato preparato per il workflow "IP Core Generation" di SoC Blockset (assegnazione interfacce AXI4/clock/reset) — non ha il parametro `IsHDLDut` né nessun altro parametro specifico SoC Blockset, e `socsysinfo.mat` non contiene mai dati lato FPGA. **Verificato e scartato un indizio plausibile**: un disallineamento (`LastTargetBoard="Custom Hardware Board"` invece di `ZedBoard` sul blocco `Software to AXI4-Stream`) è stato corretto con successo, ma il build dà lo stesso identico errore anche con una cartella progetto completamente nuova — non era la causa. Due indagini indipendenti (via script e via introspezione dell'app SoC Builder, 4 tentativi totali) convergono: **nessuna via scriptabile trovata** — l'app SoC Builder è basata su webview Chromium (si apre anche da script, ma il suo codice è p-code cifrato e la finestra non è raggiungibile per policy di sicurezza dell'ambiente di lavoro). **Serve completare il passaggio "IP Core Generation" una volta a mano (mouse/tastiera) nella finestra SoC Builder.** Dettagli completi in `docs/socbuilder_notes.md`.
- Rendere i tre fix del toolchain permanenti in modo più pulito (es. uno script di setup del progetto o uno `startup.m`) invece di richiamarli a mano a ogni sessione MATLAB.
...
- Valutare se e come usare la Pynq-Z1 come scheda reale — durante questa indagine è stata trovata e testata con successo un'API pubblica (`soc.sdk.BoardSupport`, SoC Blockset R2019b+) che registra board SoC personalizzate via script, senza wizard: il meccanismo giusto per completare la registrazione di Pynq-Z1 (finora incompleta), riusando la configurazione del target Zynq-7000. Non ancora usata per una registrazione definitiva — vedi `docs/socbuilder_notes.md`.
