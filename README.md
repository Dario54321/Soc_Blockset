...
- **Guida didattica riusabile**: [`docs/vivado_synthesis_guide/`](docs/vivado_synthesis_guide/) — spiega dal principio (concetti generali + esempio pratico) come generare HDL da un modello Simulink SoC Blockset e sintetizzarlo in Vivado reale, bypassando `socModelBuilder`. È il metodo da riusare per il vero algoritmo MPC.
...
- **Chiuso il filone di oggi con una vera sintesi Vivado** (non solo passaggio dei controlli del validatore): il DUT `ComputeCore` (il subsystem FPGA corretto secondo le 4 regole sopra) è stato generato in VHDL reale (`makehdl`, con `UseFloatingPoint=on` per gestire i segnali `double` senza cambiarne il tipo — HDL Coder non genera codice per un blocco MATLAB Function "nudo", va incapsulato in un Subsystem con `Simulink.BlockDiagram.createSubsystem`) e sintetizzato via Tcl diretto su Vivado reale: **0 LUT, 0 registri, 0 DSP**, coerente con la misura già nota per il reshape puro (il lato FPGA di questo modello ricompone lo stream ma non moltiplica — la moltiplicazione è lato software). Dettagli in [`docs/hdl_findings.md`](docs/hdl_findings.md) (Risultato 3).
...
- **Prossimo passo prioritario**: applicare gli stessi principi (canale dati, fixed-point, task/eventi, e le 4 regole di IP Core Generation sopra) all'algoritmo MPC vero, usando la sintesi Vivado diretta via Tcl come via principale — non più la demo con due matrici. La formulazione esatta dell'MPC (stato, orizzonte, solver QP) non è ancora stata definita — passo preliminare da fare prima di procedere con l'implementazione hardware.
