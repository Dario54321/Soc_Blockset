...
- **NUOVO BLOCCO, distinto dal precedente: il bitstream FPGA non si genera ancora.** Tentato `BuildType='Processor and FPGA'`/`'FPGA only'`: il software si ricompila regolarmente, ma il build fallisce con `Unrecognized field name "bit_file"`. **Causa individuata**: il subsystem `FPGA` del modello non è mai stato preparato per il workflow "IP Core Generation" di SoC Blockset (assegnazione interfacce AXI4/clock/reset) — verificato che non ha il parametro `IsHDLDut` né nessun altro parametro specifico SoC Blockset, e che `socsysinfo.mat` non contiene mai dati lato FPGA (solo lato software). Due indagini parallele indipendenti (una sulla via scriptabile, una sul codice/introspezione dell'app SoC Builder) convergono sulla stessa conclusione: **non risulta esistere una via scriptabile per completare questo passaggio** — l'app SoC Builder è basata su webview e il suo codice è p-code cifrato, illeggibile. Probabilmente serve completarlo una volta tramite interazione reale (mouse/tastiera) con la finestra SoC Builder. Un indizio non ancora verificato: il blocco `Software to AXI4-Stream` ha `LastTargetBoard = "Custom Hardware Board"` invece di `ZedBoard` — disallineamento da verificare/correggere prima di arrendersi alla via GUI. Dettagli completi in `docs/socbuilder_notes.md`.
