...
- **Prossimo passo immediato**: ripetere il build con `BuildType='Processor and FPGA'` per ottenere anche il bitstream — non ancora tentato con i fix nuovi.
- Rendere i tre fix permanenti in modo più pulito (es. uno script di setup del progetto o uno `startup.m`) invece di richiamarli a mano a ogni sessione MATLAB.
